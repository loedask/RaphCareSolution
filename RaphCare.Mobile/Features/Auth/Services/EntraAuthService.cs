...
/// <summary>
/// Microsoft Entra ID authentication using MSAL. Handles sign-up (B2C/External ID), sign-in, and secure token storage.
/// </summary>
public class EntraAuthService : IAuthService
{
    ...
    public async Task<AuthResult> SignUpWithEmailAsync(string email, string password, CancellationToken cancellationToken = default)
    {
        try
        {
            // B2C/External ID: use AcquireTokenInteractive with sign-up policy or let the user sign up in the browser.
            // For a single "sign up then verify email" flow we trigger interactive so Entra handles registration.
    ...
    public async Task<string?> GetAccessTokenAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (accounts.Count == 0)
                return null;
            var result = await _msalClient
                .AcquireTokenSilent(new[] { _options.ApiScope }, accounts[0])
                .ExecuteAsync(cancellationToken)
                .ConfigureAwait(false);
            await StoreTokensAsync(result).ConfigureAwait(false);
