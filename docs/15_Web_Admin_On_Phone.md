...
This stays the Blazor WASM portal (`RaphCare.Web` hosted by `RaphCare.Web.Host`). It is **not** a clinician MAUI app and **not** the patient phone app.
...
2. **Thin install** after the UI works on a phone. Web app manifest, icons, name, and optionally caching the WASM shell so repeat visits on clinic Wi-Fi are faster. `RaphCare.Web.Host` already maps `.webmanifest` to the right content type; nothing serves a manifest today.
...
- Structure: [`02_Solution_Structure.md`](./02_Solution_Structure.md) (`RaphCare.Web` / `RaphCare.Web.Host`).
