...
| Connect **E580** or **E585** style Bluetooth bands | Partial | Scan needs Nearby devices permission on Android 12+. Opening Devices reconnects the claimed watch when permission is already allowed. Manual Scan fills Nearby (including your saved watch row) without dropping the watch radio; tap Connect only if you need to. Connect uses the phone Bluetooth path so the app stays open; watch-SDK Connect is off after force-closes on some phones. Live Measure through the watch SDK may not work in this build. Force-stop the separate H Band app if the phone has it. Blood pressure and other monitors are later |
