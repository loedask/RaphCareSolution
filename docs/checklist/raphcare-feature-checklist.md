...
**Overall completion (this checklist):** **74%**  
**Without wearable Phase 2+ metrics** (activity / sleep / stress / temp / ECG / glucose rows): **78%**
...
## Step 5: Hospital plan value (stay, counter, money) - 68%
...
Shipped on API, Client, and Web admin. Discharged stays also show in the patient Health records list. Casualty and theatre boards are on the hospital rail. SafeCare SOS and fall alerts also sit on the hospital overview and admin home.

### Step 5 status

**68%.** Ward notes, discharge invoice, occupancy, nurse role, discharge summaries, casualty queue, theatre list, and emergency alerts on the hospital home are in. Referrals, return booking, roster, and AI draft are not started.
...
| Step 5 Hospital plan value | 68% | Emergency home in; referrals and follow-up open |
| Cross-cutting | 88% | Demo pack on Staging; E2E smoke open |
| **Overall (scored items)** | **74%** | Out of scope / open optional excluded |
| **Without wearable Phase 2+ metrics** | **78%** | Excludes activity / sleep / stress / temp / ECG / glucose rows |
...
| **Web admin** | `Pages/Admin/Hospitals/*` | Hospital ops + inpatient lifecycle + ward notes + discharge invoice + occupancy numbers + casualty + theatre + emergency home + staff patient chart (view) + visit documentation on InProgress + collection counter + waiting screens; UI en/fr/ln/sw; phone use later (responsive + thin install, not a full PWA) |
