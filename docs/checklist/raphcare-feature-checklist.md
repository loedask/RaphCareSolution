...
- [x] Appointments page
- [x] Visit page + vitals + visit-scoped clinical docs (read-only)
- [x] Tele join page
- [x] Admin dashboard
...
**100%.** Outpatient hospital admin is end-to-end on API + Client + Web, including a staff-only patient chart (view). Clinician Mobile stays out of scope. Phone use of the same web admin is later (optional); see [`../15_Web_Admin_On_Phone.md`](../15_Web_Admin_On_Phone.md).
...
| **Web admin** | `Pages/Admin/Hospitals/*` | Hospital ops + inpatient lifecycle + staff patient chart (view); UI en/fr/ln/sw; phone use later (responsive + thin install, not a full PWA) |
