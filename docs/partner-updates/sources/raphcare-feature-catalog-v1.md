...
| Consult waiting screen | Open on a TV or tablet in reception. Shows queue codes for the next consult. Does not show names. Practice, Clinic, and Hospital outpatient. | Ready |
...
| Price catalog | Edit list prices in ZAR and USD for site plans, seats, patient care, watch packages, extra video, and Clinic AI. | Ready |
...
Patients use the phone app. Practice and hospital staff use the Portal. Platform owners use Ops for wearable fleet and for editing list prices. There is no clinician phone app in this catalog. RaphCare does not place shared doctors into hospital patient lists.
...
Prices sit on the separate price list (v2). Ops can change those list amounts on the Ops Prices page.
