...
- **Clients:** Blazor WASM (staff admin) and MAUI (patient app) consume the API via RaphCare.Client; base URL and auth are configured at host level (no hardcoded base URL in client). Staff on a phone use the same web admin (responsive UI + thin home-screen install), not a clinician MAUI app. See **docs/15_Web_Admin_On_Phone.md**.
